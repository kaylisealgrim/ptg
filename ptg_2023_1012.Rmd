...
data1 - variables of interest for all wave
...
df_scaled - with variables of interest scaled (long format)


1. PTG visualizes - Should I take the log of these variables?
2. 
...
```
```{r}
wave4 <- data %>% select(contains("ss4"))
```
...
df_pred <- df_clean %>%
  filter(wave== "wave1") %>% 
  select(SUBJID, wave, pts, etv) %>% 
  rename(pts1 = pts, etv1 = etv)
df_pred$SUBJID <- as.numeric(gsub("[A-Za-z]", "", df_pred$SUBJID))
df_var <- data1 %>% 
...
```{r}
m2etv_1 <- lcmm::hlme(ptg ~ wave + etv1, mixture = ~wave + etv1, random = ~wave, subject='SUBJID', classmb = ~ etv1, ng = 2, data = df_numeric)
m3etv_1 <- lcmm::hlme(ptg ~ wave + etv1, mixture = ~wave + etv1, random = ~wave, subject='SUBJID', classmb = ~ etv1, ng = 3, data = df_numeric)
m2pts_1 <- lcmm::hlme(ptg ~ wave + pts1, mixture = ~wave + pts1, random = ~wave, subject='SUBJID', classmb = ~ pts1, ng = 2, data = df_numeric)
m3pts_1 <- lcmm::hlme(ptg ~ wave + pts1, mixture = ~wave + pts1, random = ~wave, subject='SUBJID', classmb = ~ pts1, ng = 3, data = df_numeric)
m2gun_1 <- lcmm::hlme(ptg ~ wave + gun1, mixture = ~wave + gun1, random = ~wave, subject='SUBJID', classmb = ~ gun, ng = 2, data = df_numeric)
m3gun_1 <- lcmm::hlme(ptg ~ wave + gun1, mixture = ~wave + gun1, random = ~wave, subject='SUBJID', classmb = ~ gun, ng = 3, data = df_numeric)
```
